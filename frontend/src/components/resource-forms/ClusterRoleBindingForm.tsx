...
import { FormComponentProps, ClusterRoleBindingFormData } from './index'

const defaultData: ClusterRoleBindingFormData = {
  name: '',
  namespace: 'default',
...
export { defaultData as clusterRoleBindingDefaultData }

export function ClusterRoleBindingForm({ data, onChange, namespaceReadOnly }: FormComponentProps<ClusterRoleBindingFormData>) {
  const d = { ...defaultData, ...data }

  const update = (patch: Partial<ClusterRoleBindingFormData>) => {
    onChange({ ...d, ...patch })
  }
      ...
      <Box>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }}>
          ...
          <Button
            ...
            onClick={() =>
              update({
                subjects: [...d.subjects, { kind: 'ServiceAccount', name: '', namespace: 'default' }],
              })
            }
          >
            添加主体
