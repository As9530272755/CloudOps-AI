...
if [[ "$OS_FAMILY" == "debian" ]]; then
    ...
    if [[ -d "${DEPS_DIR}/postgresql-14" ]]; then
        ...
        dpkg -i "${DEPS_DIR}"/postgresql-14/*.deb 2>/dev/null || true
    fi

    if [[ -d "${DEPS_DIR}/redis-server" ]]; then
        log_info "安装 Redis..."
        dpkg -i "${DEPS_DIR}"/redis-server/*.deb 2>/dev/null || true
    else
...
ExecStart=/usr/local/bin/node ${INSTALL_DIR}/bin/serve-frontend.js
Environment="PORT=${FRONTEND_PORT}"
Environment="ROOT=${INSTALL_DIR}/frontend/dist"
Restart=on-failure
RestartSec=5
