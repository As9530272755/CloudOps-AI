...
func (h *K8sHandler) ListResources(c *gin.Context) {
	...
	labelSelector := c.DefaultQuery("label_selector", "")
	...
	items, total, err := h.k8sService.ListResources(c.Request.Context(), uint(clusterID), kind, namespace, keyword, resourceType, labelSelector, page, limit)
	if err != nil {
